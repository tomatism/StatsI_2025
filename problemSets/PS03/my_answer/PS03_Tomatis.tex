...
\vspace{0.5cm}
\noindent Using the vectors created when storing the linear regression residuals from Q1 and Q2, a bivariate linear regression is performed. The estimated $\hat{\beta}_1$ coefficient provides an indication of the magnitude and direction of the linear relationship between the incumbent party’s overall vote share and the incumbent party’s presidential candidate’s vote share, after controlling for the difference in campaign spending between the incumbent and the challenger. The significance of the estimate suggests that it would be appropriate to control for both \texttt{difflog} and \texttt{presvote} when estimating the \texttt{voteshare} equation. 
\lstinputlisting[language=R, firstline=118, lastline=118]{PS03_Tomatis.R}  
\begin{table}[!htbp] \centering 
...
\lstinputlisting[language=R, firstline=126, lastline=134]{PS03_Tomatis.R} 
\begin{figure}[H]\centering
